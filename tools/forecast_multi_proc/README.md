...
- For batch/manual postprocessing via Slurm, prepare a task list file (see `bgc_obc_postprocess_tasks.txt`) and submit:
  `sbatch --export=ALL,REPO_ROOT=$PWD,TASK_LIST=$PWD/tools/forecast_multi_proc/bgc_obc_postprocess_tasks.txt tools/forecast_multi_proc/submit_bgc_obc_postprocess.slurm`
  (set `REMOVE_ORIGINALS_ON_SUCCESS=0` to keep originals).
