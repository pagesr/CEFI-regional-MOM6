...
- Uses 2D lon/lat from NEP_STATIC (ocean_daily.static.nc) instead of 1D xh/yh.
  * Tracers/SSH use geolon/geolat (T-points)
  * u uses geolon_u/geolat_u (U-points)
  * v uses geolon_v/geolat_v (V-points)
- No lon wrapping / sorting required (we keep native lon convention of geolon*).
  (If you later find dateline discontinuities in geolon*, we can add a wrap fix,
   but for NEP regional it’s usually fine.)

YAML additions you said you’ll add:
  GOA_STATIC: "/archive/.../ocean_daily.static.nc"
  NEP_STATIC: "/archive/.../ocean_daily.static.nc"
