...
Context / goal
--------------
We want a single 12-month OBC file per year where:
...
End result
----------
We build an intermediate xarray Dataset `ds` that contains:
  - time (CF-style: days since YYYY-MM-01)
  - time_bnds (monthly bounds)
  - zos, so, thetao, uo, vo for 12 time steps

Then we regrid `ds` to each GOA boundary segment using the Segment class:
  - seg.regrid_tracer(...) for tracers and zos
  - seg.regrid_velocity(...) for u/v
We pass time attributes/encoding into these routines so the output NetCDF
keeps consistent CF time metadata.

Notes / future options
----------------------
If we later want everything to come purely from the forecast (including t=0),
this script can be adapted easily: just change how `ds[time=0]` is populated.
...
def _attach_2d_lonlat(da, lon2d, lat2d, dims_expected=None, name="var"):
    ...
    """
    # Identify horizontal dims on da: last 2 dims
    hdims = da.dims[-2:]
...
def write_year(year, glorys_dir, nep_static, segments, variables, month, ensemble, fct_dir,rst_dir,
    ...
    # ONLY change the last time stamp (place it at end-of-month)
    ...
    # bounds (monthly start to next monthly start)
    ...
    ds = xr.Dataset(
        data_vars=dict(
            ...
            # containers for the final 12-month forcing
    ...
    # ==========================================
    # Hindcast 3D at t=0 from RESTART (same vertical levels as forecast)
    ...
    # Fill the new dataset at time index 0
    ...
    # Hindcast zos at t=0 from ocean_month.nc (hindcast output)
    ds_sfc_hind = xr.open_dataset(path.join(glorys_dir, f"{year}0101/ocean_month.nc"))
    ds['zos'][0,:,:] = np.array(ds_sfc_hind['zos'][0,:,:])

    # Clean obviously bad values (common restart missing-value convention)
    ...
    # Forecast SSH (zos) from ocean_month.nc (drop first time because t=0 is hindcast)
    ds_sfc_fcst = xr.open_dataset(path.join(fcst_hist, "ocean_month.nc"))
    ds_sfc_fcst = ds_sfc_fcst.isel(time=slice(1, None))
    
    # Forecast 3D monthly files (Feb..Dec): each file contains one time step
    liste_files = [
    ...
    # Clean missing/bad values across all times
    ds["uo"] = ds["uo"].where(ds["uo"] <= 1e10, np.nan)
    ds["vo"] = ds["vo"].where(ds["vo"] <= 1e10, np.nan)

    # Forecast zos: load full, then drop first time and insert into ds[1:12]
    ...
    # Apply NaN mask from a reference month (time index 8) onto t=0
    ...
    # Time metadata to force onto the written OBC files (Segment.regrid_* supports this)
    time_attrs = {
        "units": f"days since {int(year):04d}-{int(month):02d}-01 00:00:00",
        "long_name": "time",
        "axis": "T",
