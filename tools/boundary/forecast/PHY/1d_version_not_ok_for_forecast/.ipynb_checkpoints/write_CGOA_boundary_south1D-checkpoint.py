#!/usr/bin/env python3
"""
FAST production OBC generator (NEP10k -> GOA2.5k) using 1-D nominal axes (xh/yh/xq/yq).

Why this version:
- Much faster than using 2-D geolon/geolat curvilinear coords for xESMF.
- Matches the old GLORYS-style workflow (simple lon/lat axes) but on NEP axis names.
- Recommended when you have many years/segments/vars to generate.
...
NOTE:
- 1-D xh/yh are "nominal" and may plot weirdly on a map. That is a plotting/geometry issue.
- For scientific confidence, use the separate verification script (below) that plots NEP using 2-D
  geolon/geolat from NEP static and compares along the OBC line.

Run:
  module load nco/5.0.1   (optional, only if you use ncrcat_years)
  ./write_nep_boundary_fast_1d.py --config nep_obc_CGOA.yaml
"""
...
def write_year(year, nep_dir, segments, variables, is_first_year=False, is_last_year=False):
    ...
    ds_z = ds_z.rename({"z_l": "z"})  # keep your z rename convention

    ds_sfc = xr.open_dataset(path.join(nep_dir, f"{year}0101crpt_forc/ocean_month.nc"))

    # Adjust time for matching initial conditions (keep your original logic)
    if "time" in ds_z:
        if is_first_year:
    ...
    if "zos" in variables and "zos" in ds_sfc:
        for seg in segments:
            ...
            tracer = ds_sfc["zos"]
            # Attach 1-D nominal coords (GLORYS-style)
...
def main(config_file):
    ...
    nep_dir = cfg.get(
        "glorys_dir",  # keep your old key for compatibility
        "/archive/Dmitry.Dukhovskoy/fre/NEP/hindcast_bgc/NEPbgc_nudged_hindcast02/history/",
    )
    output_dir = cfg.get("output_dir", "./outputs_CGOA_fast1d")
    ...
    # (Optional) show which GOA edge points you’re using
    for seg in segments:
        c = seg.coords
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FAST NEP->GOA OBC using 1-D xh/yh")
