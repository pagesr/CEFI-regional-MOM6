...
How to use:
./write_glorys_boundary.py --config glorys_obc_CGOA.yaml 
"""
...
def write_year(year, glorys_dir, segments, variables, is_first_year=False, is_last_year=False):
    ds = xarray.open_dataset(path.join(glorys_dir, f'{year}0101.goa_obcs_west.nc'))
    ...
    if 'uv' in variables and 'uo' in ds and 'vo' in ds:
        for seg in segments:
           ...
           #seg.regrid_velocity(uo, vo, suffix=year, flood=False)
            #seg.regrid_velocity(uo, vo, suffix=year, flood=False, z_i_src=ds["z_i"],weight_save=True)
...
def main(config_file):
    ...
    last_year = config.get('last_year', 2011)
    glorys_dir = config.get('glorys_dir', '/work/Remi.Pages/GOA2p5k/HINDCAST')
    output_dir = config.get('output_dir', './outputs_CGOA')
...
if __name__ == '__main__':
    ...
    default_config_file = 'glorys_obc_CGOA_west.yaml'
