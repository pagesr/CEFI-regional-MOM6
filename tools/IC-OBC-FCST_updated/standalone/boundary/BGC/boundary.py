...
class Segment():
    ...
    def _mom6_order_segment_002(self, ds):
        ...
        Segment 002 is the western boundary and MOM_input defines it as
        ``I=0,J=N:0``.  The LocStream created from ``hgrid.isel(nxp=0)`` is
        naturally J=0:N (south-to-north), so reverse only the horizontal
        ``ny_segment_002`` axis immediately before writing.  Time, vertical,
