...
def write_initial(config):
    ...
    print(glorys_file)
    ds = xarray.open_dataset(glorys_file)
    ds = ds.rename({'geolon': 'lon', 'geolat': 'lat', 'zl': 'depth'})
    glorys = ds[[temp_var, sal_var, ssh_var, u_var, v_var, 'lon', 'lat']]
    ...
    ds = xarray.open_dataset(glorys_file)
    ds = ds.rename({'geolon': 'lon', 'geolat': 'lat', 'zl': 'depth'})
    glorys = ds[[temp_var, sal_var, ssh_var,'lon', 'lat']]
    ...
    flooded = xarray.merge((
        flood.flood_kara(revert[v], zdim='zl', ydim='yh', xdim='xh') for v in [temp_var, sal_var]
    ))

    # flood zos separately to avoid the extra z=0 added by flood_kara.
    flooded[ssh_var] = flood.flood_kara(revert[ssh_var], ydim='yh', xdim='xh').isel(z=0).drop('z')
